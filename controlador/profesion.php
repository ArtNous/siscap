...
include ("../modelo/profesion.class.php");
...
switch ($oper) {
	...
	case 'autocompletar':
			...
			$accion = $objeto->consultaGeneral($entidad,' where profesionDescripcion like "%' . $_GET['term'] . '%" ','profesionDescripcion', 'ASC', 0, 10);
       
            while ($row = mysql_fetch_array($objeto->resultado, MYSQL_ASSOC)) {
                $responce[] = array(
                    'label'    		 => $row['profesionDescripcion'],
                    'value'    		 => $row['profesionDescripcion'],
                    'descrip'    	 => $row['profesionDescripcion'] 
                );
            }
    ...
    default:
        ...
        $i = 0;
        while ($row = mysql_fetch_array($objeto->resultado, MYSQL_ASSOC)) {
            $responce->rows[$i]['id'] = $row['profesionDescripcion'];
            $responce->rows[$i]['cell'] = array($row['profesionDescripcion']);
            $i++;
        }
        echo json_encode($responce);
