...
include ("../modelo/cargos.class.php");
$objeto = new cargos;
...
switch ($oper) {
	...
	case 'autocompletar':
			...
			$accion = $objeto->consultaGeneral($entidad,' where cargoDescripcion like "%' . $_GET['term'] . '%" ','cargoDescripcion', 'ASC', 0, 10);
       
            while ($row = mysql_fetch_array($objeto->resultado, MYSQL_ASSOC)) {
                $responce[] = array(
                    'label'    		 => $row['cargoDescripcion'],
                    'value'    		 => $row['cargoDescripcion'],
                    'descrip'    	 => $row['cargoDescripcion'] 
                );
            }
	...
	case 'carga_select': //carga de datos para un combo   
		...
		$objeto->consultaGeneral($entidad,'', 'cargoDescripcion', 'asc', '', '');

        $combo = "<select id='cmbCargo' name='cmbCargo' class='estilo-input' >";
        $combo .= " <option value=''> ----- Por favor Seleccione ----- </option>";
        while ($row = mysql_fetch_array($objeto->resultado, MYSQL_ASSOC)) {
            $seleccion = ($_POST['opcSel']==$row['cargoDescripcion'])? 'selected="selected"' : '';	
			
			$combo .= " <option value='" . $row['cargoDescripcion']."' ".$seleccion." >" . $row['cargoDescripcion'] . "</option>";
        }
        $combo .= "</select>";
        echo $combo;
    ...
    default:
        ...
        $i = 0;
        while ($row = mysql_fetch_array($objeto->resultado, MYSQL_ASSOC)) {
            $responce->rows[$i]['id'] = $row['cargoDescripcion'];
            $responce->rows[$i]['cell'] = array($row['cargoDescripcion']);
            $i++;
        }
        echo json_encode($responce);
