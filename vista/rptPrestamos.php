<?php
    session_start();
    require('../controlador/rptPrestamos.php');
	...
	$res = $objPrest->consultar($cedula);
	$nfilasPrest = mysql_num_rows($objPrest->resultado);
    if($nfilasPrest > 0){
         while($row = mysql_fetch_array( $objPrest->resultado)){
